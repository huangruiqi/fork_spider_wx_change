...
export class WechatParser {
    ...
    /*
    * offline 从db中获取所有文章列表，拿取config中start和max之间的内容，执行parsePageItem
    * all 从第start页中进行下载 直到max
    * next为step中context.next函数
    */
    ...
    /*
    * 对于parseArticleInfo返回的WechatArticle的数组，执行遍历去crawlDetail
    */
    ...
    */
    ...
    /*
    * 缓存文章信息以及Img图片 放置cover封面图片
    * 如果源信息中meta有Html，则是已经获取过的，就是缓存到内存
    * 没有的话就去获取
    * 利用cheerio去load body 去找img的src相关属性，包括懒加载用的data-src、type等组成url从db中去找，没有到话就去search获取并缓存
    * 判断是否是封面图片，covertype的方式和img的不一样
    */
